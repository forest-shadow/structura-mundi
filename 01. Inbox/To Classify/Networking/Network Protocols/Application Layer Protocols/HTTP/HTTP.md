...
4. Затем завершить `[[HTTP Versions]]`, чтобы понять, как менялась реализация протокола.
5. После этого при необходимости перейти к `[[HTTP Proxy]]` как к соседней теме на пересечении HTTP и proxying.
