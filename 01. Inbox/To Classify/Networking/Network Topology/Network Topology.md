...
Внутри этой темы важно различать два аспекта: physical topology как физическую организацию узлов и соединений и logical topology как схему передачи данных и взаимодействия поверх инфраструктуры. При этом именованные паттерны вроде `star`, `ring`, `bus`, `mesh` и `tree` в разных контекстах могут использоваться для описания обоих аспектов.
...
- `Common Network Topologies` собирает устойчивые канонические паттерны вроде `star`, `ring`, `mesh` и `tree` как общие модели сетевой организации, которые затем можно рассматривать в физическом и логическом аспектах.
