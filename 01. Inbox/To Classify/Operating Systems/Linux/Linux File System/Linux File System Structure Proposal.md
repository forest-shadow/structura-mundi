...
- `Linux File System` теперь лучше сделать `overview`, потому что под ним уже устойчиво собираются как минимум layout-тема `Filesystem Hierarchy Standard` и link-тема `Linking Files`.
...
- отдельные заметки про every inode-related detail, если они пока помещаются в разделы `Hard Link` и `Symbolic Link`;
...
2. Проверить, когда рядом с `Linking Files` действительно нужны `inode` или `pathname resolution` как отдельные статьи.
3. Согласовать эту ветку с `[[Linux]]` и `[[Filesystem Hierarchy Standard]]`.
