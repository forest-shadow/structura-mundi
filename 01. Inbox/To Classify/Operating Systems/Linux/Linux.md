...
- `Linux File System` нужен как Linux-specific overview про то, как общая тема `File Systems` проявляется в среде Linux через иерархию каталогов, mount points, virtual file systems, links и системные соглашения.
...
- `Linking Files` нужен как вложенный overview внутри `Linux File System`, потому что тема links в Linux естественно распадается на `Hard Link` и `Symbolic Link`.
...
6. Затем читать `[[Linking Files]]`, `[[Hard Link]]` и `[[Symbolic Link]]`, если нужен механизм Linux file links.
7. Затем уже сопоставлять эту ветку с `[[Containerization]]`, если интересует Linux как основа контейнерной изоляции.
...
- [ ] Проверить, когда рядом с `Linking Files` нужны `inode` и `pathname resolution`
- [ ] Проверить `related`
